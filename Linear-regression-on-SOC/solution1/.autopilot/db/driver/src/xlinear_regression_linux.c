...
typedef struct {
    ...
    XLinear_regression_uio_map maps[ MAX_UIO_MAPS ];
} XLinear_regression_uio_info;

/***************** Variable Definitions **************************************/
static XLinear_regression_uio_info uio_info;
...
static int uio_info_read_name(XLinear_regression_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XLinear_regression_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XLinear_regression_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XLinear_regression_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XLinear_regression_Initialize(XLinear_regression *InstancePtr, const char* InstanceName) {
	XLinear_regression_uio_info *InfoPtr = &uio_info;
